...
- **The logo is the Crossbar, from Erin's Field Command brand kit (September 2026). Use it.** A solid X of two 45° bands above a detached bar, on a 32-unit grid, with an XBAR wordmark in vector outlines derived from Barlow Condensed Bold. It lives in `src/components/BrandMark.tsx` (`XbarMark`, `XbarWordmark`, drawn in `currentColor`) and as files in `public/brand/crossbar/`. Do not redraw, trace, simplify, recolour, re-letter or approximate it, never set the wordmark as live text, and do not invent a new mark because it is awkward at a size. `tests/crossbarMark.test.ts` pins the construction and keeps the component and the files identical. `public/brand/README.md` is the inventory: which asset each surface uses, how the Crossbar was built, and which older files are kept only for rollback.
...
- **The Field Command skin is not applied yet.** The Crossbar logo, icons, share card and PDF artwork are live, but the app's colours, fonts and dark scheme are still the previous palette, the sign-in screen still shows the old horse illustration, and PDF text colours are still blue. That is the next phase of the rebrand (tokens, fonts and the CSS sweep ship together so no screen goes half-dark).
