...
//   1. profile.trial_end            (ISO string, preferred)
//   2. profile.trialEnd             (ISO string)
//   3. profile.trial.end / profile.trial.trial_end  (nested, ISO string)
//   4. profile.trial_start / profile.trialStart (+ TRIAL_LENGTH_DAYS)
//   5. profile.trial.start (+ TRIAL_LENGTH_DAYS)
// The first valid ISO date wins. If the trial branch picks one of these, the
// triggers just work; if it picks something else, this is the contract to
// align on.
export function resolveTrialEndDate(profile) {
  ...
  const candidates = [
    profile.trial_end,
    profile.trialEnd,
    profile.trial && typeof profile.trial === 'object' ? profile.trial.end : null,
    profile.trial && typeof profile.trial === 'object' ? profile.trial.trial_end : null,
  ];
  ...
  const starts = [
    profile.trial_start,
    profile.trialStart,
    profile.trial && typeof profile.trial === 'object' ? profile.trial.start : null,
  ];
...
export function buildWelcomeEmail({ recipientName = '', ranchName = '' } = {}) {
  ...
  const ranchLine = ranchName
    ? `Your ranch workspace, <strong style="color:#f5f2ec;">${ranchName}</strong>, is set up and your 14-day Professional trial is running — the full plan, no card required.`
    : 'Your ranch workspace is set up and your 14-day Professional trial is running — the full plan, no card required.';
  const body = paragraphs([
    `${greeting} welcome to XBAR.`,
  ...
  const text =
    `${greeting.replace(/<[^>]*>/g, '')} welcome to XBAR.\n\n` +
...
export function buildTrialEndingEmail({ ranchName = '', trialEndDate, daysLeft } = {}) {
  ...
  const body = paragraphs([
    `Your 14-day Professional trial${ranchName ? ` for <strong style="color:#f5f2ec;">${ranchName}</strong>` : ''} ends ${dateLine}${daysLine}.`,
...
export function buildTrialExpiredEmail({ ranchName = '' } = {}) {
  const body = paragraphs([
    `Your 14-day Professional trial${ranchName ? ` for <strong style="color:#f5f2ec;">${ranchName}</strong>` : ''} has ended.`,
...
export function buildPaymentFailedEmail({ ranchName = '', planName = '', billingPortalUrl = '' } = {}) {
  ...
  const planLine = what ? ` for <strong style="color:#f5f2ec;">${what}</strong>` : '';
  const body = paragraphs([
