...
| Attach and verify the canonical custom domain            | **Repo side done; BLOCKER verified upstream** | `vercel.json` redirects `www.xbar.app → xbar.app`. Verified via the Vercel API during this audit: the `xbar-horse-management-app` project has **no custom domain attached** (only `*.vercel.app`), while `xbar.app` is registered to someone. Attaching (or, if not owned, acquiring/replacing) the domain is the single highest-priority go-live action — see `docs/GO-LIVE-CHECKLIST.md` §1    |
...
| Item                                                 | Status                      | Evidence                                                                                                                                                                                                                                                                                                                                                                     |
| ---------------------------------------------------- | --------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Consolidate the nine CSS systems                     | **Partial — honest status** | The marketing system (`landingExperience.css`, 1,723 lines) was removed from the app entirely; `followUpExperience.css` deleted with its orphaned route; duplicate font loading removed. The remaining app CSS layers still exceed one system; full consolidation is a separate visual-regression project and was deliberately not attempted in this workflow-integrity pass |
