      ...
      careRows: [
        {
          ...
          priority: 2,
          signals: [
            { key: 'coggins', label: 'Coggins', status: 'due', detail: 'Coggins overdue', dueDate: '2026-06-04' },
          ],
        },
      ],
      transferRows: [
        {
          ...
          dueDate: '2026-06-05',
          pendingCount: 1,
          reasons: ['Transfer packet missing'],
        },
      ],
      documents: [
        {
...
test('daily briefing ranks ownership and overdue work first', () => {
  const result = priorities();
  assert.equal(result.top[0]?.kind, 'Ownership');
  assert.equal(result.overdueCount, 3);
  assert.equal(result.dueCount, 4);
});
test('overdue sales follow-ups become due even when the lead is new', () => {
  const result = priorities();
  const lead = result.items.find((item) => item.id === 'sale-lead-overdue');
  assert.equal(lead?.urgency, 'Due');
  assert.equal(lead?.timing, 'Overdue');
});
test('future new leads stay clear until their follow-up approaches', () => {
  const result = priorities();
  const lead = result.items.find((item) => item.id === 'sale-lead-later');
