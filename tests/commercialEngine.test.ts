...
test('commercial features unlock at their intended plans', () => {
  ...
  assert.equal(featureGate(subscription('Ranch Ops'), 'profitIntelligence'), null);
});

test('profit intelligence ties linked spend to break-even and safe sale price', () => {
  const profile = buildHorseProfitProfile(horse, [receipt], []);
  assert.equal(profile.breakEven, 12000);
  assert.equal(profile.safeSalePrice, 13800);
  assert.equal(profile.profitLoss, 6000);
});

test('breeding revenue calculates stallion revenue and ROI', () => {
  const profile = buildBreedingRevenueProfile(horse, [receipt]);
  assert.equal(profile.stallionRevenue, 6000);
  assert.equal(profile.projectedValue, 8000);
  assert.equal(profile.totalCosts, 2500);
  assert.equal(Math.round(profile.roi), 220);
});

test('sale hold blocks missing Coggins and ownership proof', () => {
  const hold = buildSaleHold(horse, [], undefined);
  assert.equal(hold.held, true);
  assert.match(hold.reasons.join(' '), /Coggins/);
  assert.match(hold.reasons.join(' '), /Ownership record/);
