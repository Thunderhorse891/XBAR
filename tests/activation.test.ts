...
test('activation recommends the first real operating action', () => { const result = summarizeActivation({ horses: 0, documents: 0, receipts: 0, members: 0, invitations: 0, sharedListings: 0, monthlyRate: 0, billingState: 'Manual Billing' }); assert.equal(result.percent, 0); assert.equal(result.next?.id, 'horse'); assert.equal(result.complete, false); });
