...
test('dunning email with a portal URL links the portal, without one links Billing', () => {
  ...
  assert.match(withPortal.html, /https:\/\/billing\.stripe\.com\/session\/abc/);
  ...
  assert.doesNotMatch(withoutPortal.html, /https:\/\/billing\.stripe\.com/);
...
test('dunning resolves the workspace, sends once per invoice', async () => {
  ...
  assert.match(sent[0].html, /billing\.stripe\.com/);
