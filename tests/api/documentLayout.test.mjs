...
test('branded reports embed the supplied images on every page and retain all report rows', async () => {
  ...
  const bytes = await createSectionedPdf({
    ...
    sections: [{ heading: 'Horse records', lines: Array.from({ length: 75 }, (_, i) => `Horse ${i + 1}: $123`) }],
  });
  const pdf = await PDFDocument.load(bytes);
  assert.ok(pdf.getPageCount() > 1);
  for (const [index, page] of pdf.getPages().entries()) {
    const images = page.node.Resources().lookup(PDFName.of('XObject'));
    assert.ok(images.keys().length >= (index === 0 ? 3 : 2));
  }
  const text = drawnText(bytes);
  for (let i = 1; i <= 75; i++) assert.equal(text.filter((item) => item.text === `Horse ${i}`).length, 1);
  assert.equal(text.filter((item) => item.text.startsWith('Generated by XBAR')).length, pdf.getPageCount());
});
