...
function routeTitle(path: string) {
  ...
  const labels: Record<string, string> = { '/': 'Dashboard', '/landing': 'Ranch Platform', '/horses': 'Horses', '/documents': 'Document Vault', '/ownership': 'Ownership', '/medical': 'Health', '/breeding': 'Breeding', '/sales': 'Sales', '/expenses': 'Expenses', '/reminders': 'Reminders', '/assets': 'Equipment', '/weather': 'Weather', '/shared-access': 'Sale Listings', '/subscriptions': 'Subscriptions', '/settings': 'Settings', '/setup': 'Setup', '/login': 'Login' };
...
function RouteTelemetry() {
  ...
  useEffect(() => { void trackRuntimeEvent({ workspaceId: workspaceId || undefined, eventName: 'navigation.page_view', severity: 'info', payload: { pathname: location.pathname, search: location.search } }); }, [location.pathname, location.search, workspaceId]);
...
export default function App() {
  ...
  return <Router><ErrorBoundary><ToastViewport /><RouteTelemetry /><Suspense fallback={<div className="app-loading-shell">Loading XBAR...</div>}><Routes>
    ...
    <Route path="/" element={<RequireCloudAuth><RequireWorkspaceSetup><MainLayout /></RequireWorkspaceSetup></RequireCloudAuth>}>
      <Route index element={<Dashboard />} /><Route path="horses" element={<Horses />} /><Route path="horses/:id" element={<HorseDetail />} /><Route path="documents" element={<Documents />} /><Route path="weather" element={<Weather />} /><Route path="ownership" element={<Ownership />} /><Route path="medical" element={<Medical />} /><Route path="breeding" element={<Breeding />} /><Route path="sales" element={<Sales />} /><Route path="expenses" element={<Expenses />} /><Route path="reminders" element={<Reminders />} /><Route path="assets" element={<RanchAssets />} /><Route path="subscriptions" element={<Subscriptions />} /><Route path="shared-access" element={<SharedAccess />} /><Route path="settings" element={<Settings />} /><Route path="*" element={<NotFound />} />
    </Route>
    <Route path="*" element={<NotFound />} />
  </Routes></Suspense></ErrorBoundary></Router>;
}
