...
export function buildActivationSteps(input: ActivationInput): ActivationStep[] {
  ...
  return [
    { id: 'horse', title: 'Anchor the first horse', description: 'Create the command file the rest of the operation can organize around.', action: 'Add horse', path: '/horses?new=1', complete: input.horses > 0, value: `${input.horses} horse${input.horses === 1 ? '' : 's'} anchored` },
