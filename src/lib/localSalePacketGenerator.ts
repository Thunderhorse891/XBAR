...
export type PacketCredentialParams = {
  ...
  generatedBy: string;
  /**
   * The workspace profile. REQUIRED: the seller contact block is buyer-visible
   * and sealed, so the seal must resolve it from the same profile the page
   * renders — sealing with no profile covers blank name/ranch/email while the
   * page prints the real ones, and a buyer could alter the visible contact
   * block without changing the digest.
   */
