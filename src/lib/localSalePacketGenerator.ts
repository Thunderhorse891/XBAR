...
export function buildPacketCredential(params: PacketCredentialParams): SaleCredential {
  ...
  return buildSaleCredential({
    attachments: (params.attachments ?? []).map((file) => ({
      ...
      // The bytes, not the metadata. `dataUrl` is `data:<type>;base64,<data>`;
      // hashing the base64 payload alone keeps the digest independent of how
      // the MIME type happened to be recorded.
