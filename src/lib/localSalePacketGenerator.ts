...
export function buildLocalSalePacket(params: {
   ...
   */
  const sellerName = (
    params.workspaceProfile.defaultOwnerName ||
    params.workspaceProfile.ranchManagerName ||
    ''
  ).trim();
  const sellerRanch = params.workspaceProfile.ranchName.trim();
  const sellerEmail = params.workspaceProfile.operationsEmail.trim();
  const sellerByline = resolveSellerByline(params.workspaceProfile);
  const heroPhotoUrl = toPublicPassport(params.horse).photoUrl;
  const contactRows = [
