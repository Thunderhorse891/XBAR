import { buildBuyerPacketReleaseGate } from './buyerPacketReleaseGate.js';
import { toPublicPassport } from './buyerSafePassport.js';
import { type SaleCredential, buildSaleCredential } from './saleCredential.js';
...
/**
 * Who the packet says prepared it: the seller's own name and ranch, never a
 * workspace role.
 *
 * The generator used to print `Generated by ${generatedBy}`, and every caller
 * passed the workspace role — so a buyer reading the packet met "Ranch
 * Manager", a job title, not a person. A role is how the app addresses the
...
export type PacketCredentialParams = {
  ...
  generatedBy: string;
  /**
   * The workspace profile, used to resolve the seller byline the seal is
   * attributed to (see `resolveSellerByline`). Optional so callers that only
   * record a packet keep working; when absent the seal falls back to
   * `generatedBy`.
   */
...
/**
 * SHA-256 of the hero photo's bytes when they are in hand at seal time (a
 * data: URL). Remote URLs seal by URL — see CredentialSeller.heroPhotoDigest.
 */
function photoContentDigest(photoUrl: string): string {
  if (!photoUrl.startsWith('data:')) return '';
  const comma = photoUrl.indexOf(',');
  if (comma < 0) return '';
  return sha256Bytes(base64ToBytes(photoUrl.slice(comma + 1)));
}

/**
 * The seller contact block, resolved ONCE for both the seal and the render.
 *
 * The packet prints the seller name, ranch, operations email and hero photo
 * at the top; the seal must fingerprint exactly those values. Resolving them
 * separately in each place is how the printed contact details and the sealed
 * ones would come to disagree — the hole this closes. (Same pattern as the
 ...
 * seller contact block, and the full metadata of each included document —
 * plus the ownership proofs actually verified at seal time (nothing is assumed
...
export function buildPacketCredential(params: PacketCredentialParams): SaleCredential {
  ...
  // The seller contact block is buyer-visible, so it is sealed. The release
...
export function buildLocalSalePacket(params: {
  ...
  // M16: a horse with no name yet must not produce a title with a dangling em-dash.
  const horseName = shown.identity.name || 'Unnamed horse';
  const title = `XBAR Buyer Sale Packet — ${horseName}`;
  const fileName = `${cleanFileName(params.horse.name)}-xbar-buyer-sale-packet.html`;
  const disclaimer =
    'XBAR LLC generates this packet from user-supplied records. XBAR does not independently verify horse identity, ownership, registration, liens, veterinary condition, health status, sale terms, or transfer validity. Buyers and sellers must independently verify all information before payment, transport, breeding, insurance, or transfer.';
  const pendingDocuments = params.ownershipRecord?.pendingDocuments.join(', ') || '';

  /*
   * H25: the seller contact block and the horse's hero photo, at the top of
   * the packet. The photo is the buyer-safe primary photo (approved photo
   * assets only); when there is none the block renders without it — never a
   * broken image tag. Each contact field renders only when present. There is
   * no phone field on the workspace profile, so none is invented.
   *
