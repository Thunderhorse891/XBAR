...
export function scheduleBuyerActivityFollowUp(lead: SalesLead, event: BuyerRoomEvent, now = new Date()) {
  ...
  const notes = [lead.notes?.trim(), lead.notes?.includes(followUpNote) ? '' : followUpNote]
    .filter(Boolean)
    .join('\n\n');
...
export function sortFollowUps(leads: SalesLead[], now = new Date()) {
  ...
  return leads
    ...
    .sort(
      (left, right) =>
        rank[followUpTiming(left, now)] - rank[followUpTiming(right, now)] ||
        Date.parse(left.nextFollowUp || '9999-12-31') - Date.parse(right.nextFollowUp || '9999-12-31'),
    );
}
