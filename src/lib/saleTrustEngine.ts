...
function latestCurrentCoggins(documents: DocumentRecord[], horseId: string, now = new Date()) {
  return documents
    .filter((document) => document.horseId === horseId && document.type === 'Coggins' && document.state === 'Ready')
    .some((document) => {
      const date = document.entities.examDate ?? document.uploadedAt;
...
export function buildSaleHold(horse: HorseRecord, documents: DocumentRecord[], ownershipRecord?: OwnershipRecord, now = new Date()): SaleHold {
  ...
  const hasOwnershipProof = documents.some((document) =>
    ...
    (document.type === 'Registration' || document.type === 'Bill of Sale' || document.type === 'Transfer Packet' || document.type === 'Ownership Memo'));

  if (horse.status === 'Medical Review') reasons.push('Medical Review blocks sale release.');
  if (!latestCurrentCoggins(documents, horse.id, now)) reasons.push('Current Coggins is missing.');
