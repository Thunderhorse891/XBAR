...
export function buildOperationsPriorities(input: PriorityInput, now = new Date()): OperationsPrioritySummary {
  const items: OperationsPriorityItem[] = [
      ...
      row.signals
        ...
        .map((signal) =>
          withPriority(
            ...
            now,
            signal.status === 'due' ? 8 : 0,
          ),
        ),
    ),
    ...input.transferRows.map((gap) =>
      withPriority(
        {
    ...
    ...input.documents
      .filter((document) => document.state === 'Needs Review' || document.state === 'Matched')
      .map((document) =>
        withPriority(
          {
            ...
            detail: document.horseId
              ? 'Approve or archive this matched file.'
              : 'Assign this file to a horse before it becomes trusted record proof.',
            route: '/documents',
          },
          now,
          document.state === 'Needs Review' ? 6 : 0,
        ),
      ),
    ...input.salesLeads
      ...
      .map((lead) => {
        ...
        const urgency: ReminderUrgency =
          offset !== null && offset <= 0
            ? 'Due'
            : offset !== null && offset <= 7
              ? 'Watch'
              : lead.stage === 'Offer' || lead.stage === 'Qualified'
                ? 'Watch'
                : 'Clear';
    ...
    (left, right) =>
      ...
      Date.parse(left.dueDate || '9999-12-31') - Date.parse(right.dueDate || '9999-12-31'),
  );
  return {
    items,
