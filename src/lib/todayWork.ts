...
function setupTasks(input: TodayWorkInput): WorkTask[] {
  ...
  if (input.horses.length > 0 && input.documents.filter((document) => document.state !== 'Archived').length === 0) {
    tasks.push({
      ...
      category: 'Documents',
      detail:
        'Upload registration, Coggins, health, or ownership proof so the horse record has trusted source material.',
  ...
  if (input.horses.length > 0 && input.sharedListings.length === 0 && input.salesLeads.length === 0) {
    tasks.push({
      ...
      category: 'Sales',
      detail:
        'Create a controlled listing or buyer-ready packet from a real horse record when the documents are ready.',
      route: '/sales',
      source: 'Workspace Setup',
    });
  }
...
function sortTasks(left: WorkTask, right: WorkTask) {
  ...
  const dueRank = (task: WorkTask) =>
    task.due === 'Now' ? 0 : task.due === 'Overdue' ? 1 : task.due === 'Today' ? 2 : task.due === 'This week' ? 3 : 4;
  return (
...
export function buildTodayWork(input: TodayWorkInput): WorkTask[] {
  ...
  const allTasks = [
    ...
    ...priorities.items
      .filter((item) => item.urgency !== 'Clear')
      .map((item) => priorityItemToTask(item, input.workspaceProfile)),
  ];
