...
export function buildUsageMeters(subscription: SubscriptionProfile): UsageMeter[] {
  ...
  // Seats meter is only meaningful on plans where team invites are available.
  // On Starter (1 seat), the workspace admin fills the slot immediately, making
  // the meter show "blocked" before the user has done anything wrong.
  if (featureGate(subscription, 'teamInvites') === null) {
    inputs.push({ key: 'seats', label: 'Team seats', used: usage.seatsUsed, limit: usage.seatLimit });
  }
