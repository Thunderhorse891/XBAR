...
export function hasMeaningfulWorkspace(backup: unknown) {
  ...
  const hasProfile = ['setupCompleteAt', 'businessName', 'ranchName', 'defaultOwnerName', 'operationsEmail'].some((key) => typeof profile?.[key] === 'string' && String(profile[key]).trim());
  return hasProfile || ['horses', 'documents', 'intakeBatches', 'expenseReceipts', 'ranchAssets', 'salesLeads', 'sharedListings', 'workspaceMembers', 'workspaceInvitations'].some((key) => Array.isArray(workspace[key]) && workspace[key].length > 0);
}
...
export function isMissingCloudWorkspaceMessage(message: string) {
  ...
  return normalized.includes('no cloud workspace') || normalized.includes('no relational workspace') || normalized.includes('no relational workspace records');
}

export function decideCloudReconciliation(params: { local: unknown; remote?: unknown; remoteError?: string }) : CloudReconciliation {
  ...
  if (params.remote !== undefined) {
    ...
    return serializeWorkspaceBackup(params.local) === serializeWorkspaceBackup(params.remote) ? 'connected' : 'conflict-lock';
  }
