      ...
      <div className="dashboard-board">
        ...
        <aside className="dashboard-board__side">
          ...
          <Panel title="Buyer desk" action={<Link to="/sales" className="button button--ghost button--compact">Buyer Desk</Link>}>
            {salesLeads.length ? <div className="stack-list">{salesLeads.slice(0, 4).map((lead) => { const horse = horses.find((item) => item.id === lead.horseId); return <button key={lead.id} type="button" className="stack-item stack-item--interactive" onClick={() => navigate('/sales')} onContextMenu={(event) => { event.preventDefault(); setMenuState({ type: 'lead', id: lead.id, x: event.clientX, y: event.clientY }); }}><div className="stack-item__top"><div className="stack-item__title">{lead.name}</div><Pill tone={lead.stage === 'Offer' ? 'blue' : 'slate'}>{lead.stage}</Pill></div><div className="stack-item__copy">{horse?.name ?? 'No horse assigned'} · {lead.nextFollowUp ? `Follow up ${formatDateLabel(lead.nextFollowUp)}` : 'No follow-up date'}</div></button>; })}</div> : <EmptyState compact title="No buyer movement" description="Buyer activity appears here once leads are attached to command files." />}
          </Panel>
