...
export function ActivationGuide() {
  ...
  return <section className="activation-guide" aria-labelledby="activation-title"><div className="activation-guide__header"><div><span className="activation-guide__eyebrow">Setup Plan</span><h2 id="activation-title">{activation.firstValueAchieved ? 'Your first horse record is taking shape.' : 'Reach the first useful outcome quickly.'}</h2><p>{activation.valueStatement} Then keep adding the context that makes XBAR useful every day.</p></div><div className="activation-guide__score"><strong>{activation.percent}%</strong><span>complete</span><button type="button" onClick={toggle}>Collapse</button></div></div><div className="activation-guide__progress"><span style={{ width: `${activation.percent}%` }} /></div><div className={`activation-value${activation.firstValueAchieved ? ' activation-value--reached' : ''}`}><div><span>{activation.firstValueAchieved ? 'First value reached' : 'First value target'}</span><strong>{activation.firstValueAchieved ? 'One horse now has a connected document.' : 'Connect one horse and one source record.'}</strong><p>{activation.valueStatement}</p></div><Link className="button button--primary button--compact" to={activation.firstValueAchieved ? '/horses' : activation.next?.path ?? '/horses'} onClick={() => trackStep('first-value', activation.firstValueAchieved ? '/horses' : activation.next?.path ?? '/horses')}>{activation.firstValueAchieved ? 'Open horse records' : activation.next?.action}</Link></div><div className="activation-guide__steps">{activation.steps.map((step, index) => <article className={`activation-step${step.complete ? ' activation-step--complete' : step.id === activation.next?.id ? ' activation-step--next' : ''}`} key={step.id}><div className="activation-step__number">{step.complete ? '✓' : index + 1}</div><div className="activation-step__copy"><strong>{step.title}</strong><span>{step.description}</span><small>{step.value}</small></div>{step.complete ? <span className="activation-step__done">Complete</span> : <Link className="button button--ghost button--compact" to={step.path} onClick={() => trackStep(step.id, step.path)}>{step.action}</Link>}</article>)}</div></section>;
}
