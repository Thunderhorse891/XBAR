...
import { buildUpgradePressure, buildUsageMeters, formatUsageValue, getUpgradePath, usageMeterTone } from '@/lib/subscriptionUsage';
...
export function UsageMeterPanel({ compact = false }: { compact?: boolean }) {
  ...
  const meters = useMemo(() => buildUsageMeters({ subscription, horsesUsed: horses.length, documentsUsed: documents.length, salePacketsGenerated: salePacketCount }), [documents.length, horses.length, salePacketCount, subscription]);
