        ...
        <Card>
          <div className="xs-empty">
            ...
            <div className="xs-empty__sub">
              Add horses and XBAR groups them automatically — sale prospects, broodmares, studs, and young stock.
            </div>
            <ActionButton variant="primary" icon={<Plus size={15} />} onClick={() => navigate('/horses?new=1')}>
              Add first horse
            </ActionButton>
        ...
        {groups.map((g) => (
          <Card key={g.id}>
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 10 }}>
              <div>
                <h2 className="xs-card__title" style={{ fontSize: 18 }}>
                  {g.name}
                </h2>
                <div className="xs-card__sub">{g.location}</div>
              </div>
              <StatusChip tone={g.needsCare ? 'warning' : 'neutral'}>
                {g.needsCare ? `${g.needsCare} need care` : 'OK'}
              </StatusChip>
            </div>
            <div className="xs-statgrid" style={{ marginTop: 14 }}>
              <div className="xs-stattile">
                <div className="xs-stattile__num">{g.count}</div>
                <div className="xs-stattile__label">Horses</div>
              </div>
              <div className="xs-stattile">
                <div className={`xs-stattile__num${g.needsCare ? ' xs-stattile__num--warning' : ''}`}>
                  {g.needsCare}
                </div>
                <div className="xs-stattile__label">Need care</div>
              </div>
            </div>
