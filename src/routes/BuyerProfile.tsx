    ...
    <main className="buyer-shell">
      <div className="buyer-shell__inner">
        ...
        <div className="metric-grid">
          ...
          <MetricCard
            label="Packet status"
            value={packet.score >= 80 ? 'Ready' : packet.score >= 50 ? 'Partial' : 'Incomplete'}
            detail={packet.score >= 80 ? 'All key documents on file' : `${Math.round(packet.score)}% coverage — contact for details`}
            tone={packet.score >= 80 ? 'emerald' : packet.score >= 50 ? 'amber' : 'rose'}
          />
          <MetricCard
