      ...
      <>
        ...
        <Card>
          <div className="xs-empty">
            <span className="xs-empty__icon"><HeartPulse size={26} /></span>
            <div className="xs-empty__title">No care records yet</div>
            <div className="xs-empty__sub">Add horses and their health records — XBAR tracks Coggins, wormer, dental, and vaccine due dates and flags what's holding up a sale.</div>
  ...
  return (
    <>
      ...
      <div className="xs-grid-3">
        <Card><div className="xs-card__sub">Care due</div><div style={{ fontSize: 28, fontWeight: 700, color: dueCount ? 'var(--xbar-danger)' : 'var(--xbar-text)' }}>{dueCount}</div></Card>
        <Card><div className="xs-card__sub">Watch</div><div style={{ fontSize: 28, fontWeight: 700, color: watchCount ? 'var(--xbar-warning)' : 'var(--xbar-text)' }}>{watchCount}</div></Card>
        <Card><div className="xs-card__sub">Medical holds</div><div style={{ fontSize: 28, fontWeight: 700, color: medicalHolds ? 'var(--xbar-danger)' : 'var(--xbar-text)' }}>{medicalHolds}</div></Card>
      </div>
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.id} onClick={() => navigate(`/animals/${r.horseId}`)}>
                ...
                <td><StatusChip tone={STATUS_TONE[r.status]}>{STATUS_LABEL[r.status]}</StatusChip></td>
              </tr>
            ))}
          </tbody>
        </table>
      </div>
      {rows.length === 0 ? <Card><div className="xs-empty">No care items match this filter.</div></Card> : null}
    </>
  );
}
