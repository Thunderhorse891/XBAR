...
import './xbarLoginRefresh.css';

type AuthMode = 'signin' | 'signup';
type BusyState = 'password' | 'google' | 'facebook' | 'apple' | 'reset' | '';
const features = ['Horse records', 'Buyer packets', 'Deal rooms', 'Proof vault', 'Plan control'];
...
export default function Login() {
  ...
  const description = !supabaseReady ? 'Explore the complete operating workflow in this browser before cloud account services are enabled.' : selectedPlan ? `Create your workspace, then review ${selectedPlan} in secure checkout.` : 'Your ranch, horses, records, and buyer movement in one command system.';

  return <main className="xbar-login-shell lp-shell"><div className="xbar-login-noise" aria-hidden="true" /><div className="lp-frame"><section className="lp-brand" aria-labelledby="login-brand-title"><div className="brand-canvas" aria-hidden="true"><div className="brand-canvas__lockup"><span className="brand-canvas__x">X</span><span className="brand-canvas__bar">XBAR</span><span className="brand-canvas__rule" /></div></div><div className="lp-tagline"><strong id="login-brand-title">Equine operations command.</strong><span>Engineered for serious horse businesses.</span></div><div className="lp-features">{features.map((feature) => <div className="lp-feature" key={feature}>{feature}</div>)}</div><p className="lp-card-sub">Built for breeders, ranches, boarding facilities, training operations, and buyer-ready sale workflows.</p></section>
