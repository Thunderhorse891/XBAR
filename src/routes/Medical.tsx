...
export default function Medical() {
  ...
  const menuEvent =
    menuState?.type === 'event'
      ? medicalEvents.find((event) => event.id === menuState.eventId && event.horseId === menuState.horseId)
      : undefined;
  ...
  const openEventDetails = (event: (typeof medicalEvents)[number]) =>
    openRightDrawer({
  ...
  const openEventMenu = (horseId: string, eventId: string, x: number, y: number) =>
    setMenuState({ type: 'event', horseId, eventId, x, y });
        ...
        ...(canManageMedical
          ? [
              ...
              },
              {
                ...
                onSelect: () => {
                  setPendingDelete({
                    horseId: menuEvent.horseId,
                    eventId: menuEvent.id,
                    horseName: menuEvent.horseName,
                  });
                },
              },
            ]
  ...
  return (
    <>
      ...
      <div className="metric-grid">
        <MetricCard
          label="Watchlist"
          value={`${medicalWatch.length}`}
          detail="Horses needing care attention"
          tone="rose"
        />
      ...
      <div className="dashboard-grid dashboard-grid--primary">
        <Panel eyebrow="On Watch" title="Watch">
          {medicalWatch.length ? (
            ...
            <EmptyState
              compact
              title="No horses on medical watch"
              description="Add a care event to start the watchlist."
            />
          )}
        </Panel>
      ...
      <ContextMenu
        open={Boolean(menuHorse || menuEvent)}
        x={menuState?.x ?? 0}
        y={menuState?.y ?? 0}
        items={menuItems}
