...
import { getPlanCapacity } from '@/lib/subscriptionUsage';
...
export default function Subscriptions() {
  ...
  return <>
    ...
    <section className="subscription-hero">
      <div className="subscription-hero__main"><Pill tone={requestedTier ? 'emerald' : 'blue'}>{requestedTier ? `${requestedTier} selected` : 'Built to grow with the operation'}</Pill><h2 className="subscription-hero__title">{requestedTier ? `Review ${requestedTier}, then start securely.` : 'Choose capacity around the work that must move.'}</h2><p className="subscription-hero__copy">XBAR should make money by solving expensive barn problems: client payments, care accountability, documents, inventory, scheduling, owner visibility, breeding operations, and clean exports. Prices stay fixed; each tier now has a clearer revenue job.</p><div className="subscription-trust-row"><span>Existing prices preserved</span><span>Upgrade when work expands</span><span>Your records stay intact</span></div></div>
    ...
    <section className="subscription-decision" aria-label={`${decisionTier} decision framework`}>
      <div className="subscription-decision__main"><span className="subscription-decision__eyebrow">What {decisionTier} changes</span><h3>{decisionTier === subscription.tier ? `${decisionTier} remains the right operating level.` : `Move to ${decisionTier} when these outcomes matter now.`}</h3><p>{decisionProfile.revenueRole}</p><div className="subscription-decision__outcomes">{planOutcomes[decisionTier].map((outcome) => <span key={outcome}>{outcome}</span>)}</div></div>
    ...
    <Panel eyebrow="Choose your operating level" title="Plans"><div className="subscription-plan-grid">{tiers.map((tier) => {
      ...
      const capacity = getPlanCapacity(tier);
